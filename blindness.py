...
def OpenFile():
    ...
    if y:
        try:
            ...
            query = 'UPDATE user_info SET PREDICT = "%s" WHERE USERNAME = "%s"'%(value, username)
...
def Signup():
    ...
    if u:
        ...
        if z:
            query = "INSERT INTO user_info (USERNAME, PASSWORD) VALUES('%s', '%s')" % (username, password)
...
label1 = Label(root, text="Blindness Detection", font=('Arial', 30))
